...
-- | Query the client
query :: forall e a. Command -> Client -> AffResult e a
...
query' :: forall e a
  ...
  -> (a -> Eff (db :: DB | e) Unit)
  -> (Eff (db :: DB | e) (Canceler (db :: DB | e)))
query' q c eb cb = runFn5 _query (runCommand q) c ignoreCancel eb cb
...
foreign import _query
  ...
  function _query(query, client, canceler, errback, callback) {
    var Command = require('ioredis/lib/command'),
        ...
        command = new Command(query.name, query.args, options, function(err, x) {
